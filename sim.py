...
class City:
    def __init__(self, pos):
        ...
        for k in get_adj(pos, bounds=WorldMap.keys()):
            if WorldMap[k]["Terrain"] in ["Sea", "Ocean"]:
                self.resource_gathering["Fish"] = (self.resource_gathering["Fish"] + 0.1 if "Fish" in self.resource_gathering.keys() else (0 + 1) * 0.1)
    ...
    def tick(self):
        ...
        for item in Animals.keys():
            self.resources[f"Tame {item}"] = math.floor(
                (self.resources[f"Tame {item}"] if f"Tame {item}" in self.resources.keys() else 0) * 0.9)  # Animals die periodically
        ...
        for item in self.resource_gathering.keys():
            ...
            production = \
                (self.resource_gathering[item] * config["MAX_PRODUCTION_CONSTANT"]) * ((self.population + self.homunculi) * 2) / (self.resource_gathering[item] * config["MAX_PRODUCTION_CONSTANT"] + (self.population + self.homunculi) * 2)
            if production < 0:
                print(production)
            self.resources[item] = (self.resources[item] + production if item in self.resources.keys() else (0 + 1) * production)
            self.production[item] = (self.production[item] + production if item in self.production.keys() else (0 + 1) * production)
            if item in ["Fish", *Plants.keys(), *Animals.keys()]:
                food_resources[item] = (self.resources[item] if item in self.resources.keys() else 0)
        ...
        for item in AllItems:
            ...
            if (demand[item] if item in demand.keys() else 0) == 0:
        ...
        for demanded_item in demand.keys():
            ...
            demand[demanded_item] -= (self.resources[demanded_item] if demanded_item in self.resources.keys() else 0)
            self.economy[demanded_item] = 1
            mat = (demanded_item[5:] if demanded_item[:5] == 'Tame ' else demanded_item[4:] if demanded_item[:4] == 'Cut ' else demanded_item[:-6] if demanded_item[-6:] == ' Goods' else demanded_item)
            ...
            if demanded_item == Magic["Material"][0]:
                if (self.resources[demanded_item] if demanded_item in self.resources.keys() else 0) - config["MAGIC_CONSUMPTION"] * (
                        demand[demanded_item] + (self.resources[demanded_item] if demanded_item in self.resources.keys() else 0)) < 0:
                    self.resources[demanded_item] = 0
                else:
                    self.resources[demanded_item] = (self.resources[demanded_item] - config["MAGIC_CONSUMPTION"] * (demand[demanded_item] + (self.resources[demanded_item] if demanded_item in self.resources.keys() else 0)) if demanded_item in self.resources.keys() else (0 - 1) * config["MAGIC_CONSUMPTION"] * (demand[demanded_item] + (self.resources[demanded_item] if demanded_item in self.resources.keys() else 0)))
                continue
            net_food -= demand[demanded_item]
            net_food -= (self.resources[demanded_item] if demanded_item in self.resources.keys() else 0) / 2
        ...
        if not alive_ruler:
            ...
            if alive_npcs:
                ...
                if new_ruler.race == self.majority_race or random.random() < 0.05 * \
                        (new_ruler.skills["Leadership"] if "Leadership" in new_ruler.skills.keys() else 0) or new_ruler.pos == self.pos:
        ...
        for c in CityList:
            ...
            for army_n in WorldMap[c]["Structure"].armies:
                if army["Pos"] == army_n["Pos"] and army_n["Size"] > 0 and army_n["Origin"] != self.pos and \
                        sum((self.cultural_values[k] - other_city.cultural_values[k]) ** 2 for k in self.cultural_values.keys()) > 45:
    ...
    def describe_culture(self):
        ...
        if len(refactored_values[1]) != 0:
            addition = (refactored_values[1][0] if len(refactored_values[1]) == 1 else refactored_values[1][0] + ' and ' + refactored_values[1][1] if len(refactored_values[1]) == 2 else (', '.join(refactored_values[1][:-1]) + ', and ' + refactored_values[1][-1])).lower() + (
        ...
        if len(refactored_values[2]) != 0:
            addition = (refactored_values[2][0] if len(refactored_values[2]) == 1 else refactored_values[2][0] + ' and ' + refactored_values[2][1] if len(refactored_values[2]) == 2 else (', '.join(refactored_values[2][:-1]) + ', and ' + refactored_values[2][-1])).lower() + (
                " is" if len(refactored_values[2]) == 1 else " are") + " considered useless. "
            description += addition[0].upper() + addition[1:]
        if len(refactored_values[3]) != 0:
            addition = (refactored_values[3][0] if len(refactored_values[3]) == 1 else refactored_values[3][0] + ' and ' + refactored_values[3][1] if len(refactored_values[3]) == 2 else (', '.join(refactored_values[3][:-1]) + ', and ' + refactored_values[3][-1])).lower() + (
                " is" if len(refactored_values[3]) == 1 else " are") + " viewed neutrally. "
            description += addition[0].upper() + addition[1:]
        if len(refactored_values[4]) != 0:
            addition = (refactored_values[4][0] if len(refactored_values[4]) == 1 else refactored_values[4][0] + ' and ' + refactored_values[4][1] if len(refactored_values[4]) == 2 else (', '.join(refactored_values[4][:-1]) + ', and ' + refactored_values[4][-1])).lower() + (
                " is" if len(refactored_values[4]) == 1 else " are") + " important. "
            description += addition[0].upper() + addition[1:]
        if len(refactored_values[5]) != 0:
            addition = (refactored_values[5][0] if len(refactored_values[5]) == 1 else refactored_values[5][0] + ' and ' + refactored_values[5][1] if len(refactored_values[5]) == 2 else (', '.join(refactored_values[5][:-1]) + ', and ' + refactored_values[5][-1])).lower() + (
...
class NPC:
    ...
    def tick(self):
        ...
        if (self.age - self.lifespan) / self.lifespan > random.random():
            ...
            if self.pos == self.origin:
                WorldMap[self.origin]["Structure"].resources = \
                    {k: (self.inventory[k] if k in self.inventory.keys() else 0) + (WorldMap[self.origin]["Structure"].resources[k] if k in WorldMap[self.origin]["Structure"].resources.keys() else 0) for k in [*self.inventory.keys(), *WorldMap[self.origin]["Structure"].resources.keys()] }
                self.inventory = {}
            return
        ...
        if self.pos != self.origin and traveler_options != []:
            ...
            for monster in (_.monster for _ in RegionList):
                if self.pos == monster.location and monster.alive:  # Combat
                    luck = random.random()
                    result = luck / (1 - luck) + (self.skills["Adventuring"] if "Adventuring" in self.skills.keys() else 0)
                    if (self.skills[Magic["Name"]] if Magic["Name"] in self.skills.keys() else 0) > 0:
                        for k in Magic["Abilities"]:
                            ...
                            if len(ability_res) > 0:
                                ...
                                self.inventory[ability_res[0][0]] = (self.inventory[ability_res[0][0]] - amount if ability_res[0][0] in self.inventory.keys() else (0 - 1) * amount)
                                # print(ability_res[0][0], ability_res[0][1])
                                result += amount
                    self.reputation += 2
                    if result < 8:  # Monster kills NPC
                        self.alive = False
                        monster.inventory = {k: (self.inventory[k] if k in self.inventory.keys() else 0) + (monster.inventory[k] if k in monster.inventory.keys() else 0) for k in [*self.inventory.keys(), *monster.inventory.keys()] }
                        self.inventory = {}
                        ...
                        self.inventory = {k: (self.inventory[k] if k in self.inventory.keys() else 0) + (monster.inventory[k] if k in monster.inventory.keys() else 0) for k in [*self.inventory.keys(), *monster.inventory.keys()] }
                        monster.inventory = {}
                        monster.alive = False
                        self.reputation += 2
            else:
                if self.pos in CityList:  # Enter another city
                    ...
                    culture_distance = sum((my_city.cultural_values[k] - other_city.cultural_values[k]) ** 2 for k in my_city.cultural_values.keys())
                    cultural_differences = {k: abs(v - other_city.cultural_values[k]) for k, v in
                    ...
                    for k, v in my_city.cultural_values.items():
                        ...
                        difficulty = culture_distance * cultural_differences[k] - \
                                     (self.skills["Philosophy"] if "Philosophy" in self.skills.keys() else 0)
                        luck = random.random()
        ...
        if self.age > 15 and random.random() * 10 < (
                (self.skills["Adventuring"] if "Adventuring" in self.skills.keys() else 0) / self.age) and traveler_options != []:
            self.pos = random.choice(traveler_options)  # Travel to a random location within the region
            ...
            for k in Magic["Abilities"]:
                ...
                if k["Type"] == "Combat":
                    ...
                    if len(ability_res) > 0:
                        requested_magic[ability_res[0][1]] = (requested_magic[ability_res[0][1]] + k["Strength"] if ability_res[0][1] in requested_magic.keys() else (0 + 1) * k["Strength"])
                        print(ability_res[0][0], ability_res[0][1])
            for k in requested_magic.keys():
                requested_magic[k] = (requested_magic[k] - (self.inventory[k] if k in self.inventory.keys() else 0) if k in requested_magic.keys() else (0 - 1) * (self.inventory[k] if k in self.inventory.keys() else 0))
                amount = min(requested_magic[k], (WorldMap[self.origin]["Structure"].resources[k] if k in WorldMap[self.origin]["Structure"].resources.keys() else 0))
                if amount > 0:
                    self.inventory[k] = (self.inventory[k] + amount if k in self.inventory.keys() else (0 + 1) * amount)
                    WorldMap[self.origin]["Structure"].resources[k] = (WorldMap[self.origin]["Structure"].resources[k] - amount if k in WorldMap[self.origin]["Structure"].resources.keys() else (0 - 1) * amount)
            return

        # Learning / Studying
        if self.age > 15:
            study_choices = {k: 0.1 + (self.skills[k] if k in self.skills.keys() else 0) % 12
            ...
            if study in library.keys():
                for k, v in library[study].items():
                    if int(k) <= (self.skills[study] if study in self.skills.keys() else 0):
                        continue
                    idol = v / (int(k) - (self.skills[study] if study in self.skills.keys() else 0))
                    break
                else:
                    write = True
            else:
                write = True
            difficulty = (self.age ** 2) * (self.skills[study] if study in self.skills.keys() else 0) / 100 / \
                         max(idol - (self.skills[study] if study in self.skills.keys() else 0), 10)
            luck = random.random()
            if luck / (1 - luck) > difficulty:  # This can be anywhere from 0 to infinity
                self.skills[study] = (self.skills[study] + 1 if study in self.skills.keys() else (0 + 1) * 1)
                ...
                if write:
                    if study in library.keys():
                        if (library[study][str(self.skills[study])] if str(self.skills[study]) in library[study].keys() else 0) < \
                                (self.skills["Teaching"] if "Teaching" in self.skills.keys() else 0):
                            library[study][str(self.skills[study])] = self.skills["Teaching"]
                    else:
                        library[study] = {self.skills[study]: (self.skills["Teaching"] if "Teaching" in self.skills.keys() else 0)}

            # Using productive skills
            metalwork_prod = (self.skills["Metalworking"] if "Metalworking" in self.skills.keys() else 0)
            ...
            if metalwork_prod > 0:
                ...
                for k, v in metalwork_res.items():  # Metalworking
                    ...
                    WorldMap[self.origin]["Structure"].resources[k] = (WorldMap[self.origin]["Structure"].resources[k] - current_goods if k in WorldMap[self.origin]["Structure"].resources.keys() else (0 - 1) * current_goods)
                    WorldMap[self.origin]["Structure"].resources[k + " Goods"] = (WorldMap[self.origin]["Structure"].resources[k + " Goods"] + current_goods if k + " Goods" in WorldMap[self.origin]["Structure"].resources.keys() else (0 + 1) * current_goods)
                    WorldMap[self.origin]["Structure"].production[k + " Goods"] = (WorldMap[self.origin]["Structure"].production[k + " Goods"] + current_goods if k + " Goods" in WorldMap[self.origin]["Structure"].production.keys() else (0 + 1) * current_goods)
                    if metalwork_prod <= 0:
                        break

            gemwork_prod = (self.skills["Gemcutting"] if "Gemcutting" in self.skills.keys() else 0)
            if gemwork_prod > 0:
                ...
                for k, v in gemwork_res.items():  # Gemcutting
                    ...
                    WorldMap[self.origin]["Structure"].resources[k] = (WorldMap[self.origin]["Structure"].resources[k] - current_goods if k in WorldMap[self.origin]["Structure"].resources.keys() else (0 - 1) * current_goods)
                    WorldMap[self.origin]["Structure"].resources["Cut " + k] = (WorldMap[self.origin]["Structure"].resources["Cut " + k] + current_goods if "Cut " + k in WorldMap[self.origin]["Structure"].resources.keys() else (0 + 1) * current_goods)
                    WorldMap[self.origin]["Structure"].production["Cut " + k] = (WorldMap[self.origin]["Structure"].production["Cut " + k] + current_goods if "Cut " + k in WorldMap[self.origin]["Structure"].production.keys() else (0 + 1) * current_goods)
                    if gemwork_prod <= 0:
                        break

            animal_prod = (self.skills["Animal Training"] if "Animal Training" in self.skills.keys() else 0)
            if animal_prod > 0:
                ...
                for k, v in animal_res.items():  # Animal Training
                    ...
                    WorldMap[self.origin]["Structure"].resources[k] = (WorldMap[self.origin]["Structure"].resources[k] - current_goods if k in WorldMap[self.origin]["Structure"].resources.keys() else (0 - 1) * current_goods)
                    WorldMap[self.origin]["Structure"].resources["Tame " + k] = (WorldMap[self.origin]["Structure"].resources["Tame " + k] + current_goods if "Tame " + k in WorldMap[self.origin]["Structure"].resources.keys() else (0 + 1) * current_goods)
                    WorldMap[self.origin]["Structure"].production["Tame " + k] = (WorldMap[self.origin]["Structure"].production["Tame " + k] + current_goods if "Tame " + k in WorldMap[self.origin]["Structure"].production.keys() else (0 + 1) * current_goods)
                    if animal_prod <= 0:
                        break

            magic_prod = (self.skills[Magic["Name"]] if Magic["Name"] in self.skills.keys() else 0)
            if magic_prod > 0:
                magic_types = [*(k for k in Magic["Abilities"] if k["Type"] in ["Homunculus", "Youth", "Portal"]
                                 and self.skills[Magic["Name"]] > (k["Min Level"] if "Min Level" in k.keys() else 0))]
                ...
                for k in magic_types:  # Magic
                    ...
                    for kk, vv in ability_res.items():
                        ...
                        WorldMap[self.origin]["Structure"].resources[k["Component"]] = (WorldMap[self.origin]["Structure"].resources[k["Component"]] - current_goods * k["Strength"] if k["Component"] in WorldMap[self.origin]["Structure"].resources.keys() else (0 - 1) * current_goods * k["Strength"])
    ...
    def tick(self, region):
        ...
        if self.location in CityList:  # Raid a city
            ...
            CityRes = {k: v * (Goods[k][1] if k in Goods.keys() else (
                    4 * (Goods[(k[5:] if k[:5] == 'Tame ' else k[4:] if k[:4] == 'Cut ' else k[:-6] if k[-6:] == ' Goods' else k)][1] if (k[5:] if k[:5] == 'Tame ' else k[4:] if k[:4] == 'Cut ' else k[:-6] if k[-6:] == ' Goods' else k) in Goods.keys() else 1)))
            ...
            for i in range(min(3, len(CityRes))):
                ...
                city.resources[res] = (city.resources[res] - amount if res in city.resources.keys() else (0 - 1) * amount)
                self.inventory[res] = (self.inventory[res] + amount if res in self.inventory.keys() else (0 + 1) * amount)
...
def manage_trade(route):
    ...
    c1_supply = {k: (c1_demand[k] if k in c1_demand.keys() else 0) * (c2.economy[k] if k in c2.economy.keys() else 0) for k in c1_keys}
    c2_supply = {k: (c2_demand[k] if k in c2_demand.keys() else 0) * (c1.economy[k] if k in c1.economy.keys() else 0) for k in c2_keys}
    # print("Demand")
    # print(c1_demand)
    # print(c2_demand)
    # print("Economy")
    ...
    # print(c1.resources)
    # print(c2.resources)
    if sum((c1.cultural_values[k] - c2.cultural_values[k]) ** 2 for k in c1.cultural_values.keys()) > 50:
        return
    ...
    try:
        ...
        if c1_trade[1] > (config["TRADE_VOLUME"] - config["TRADE_THRESHOLD"]) or c2_trade[1] > (
                ...
                (c1.resources[c2_trade[0]] if c2_trade[0] in c1.resources.keys() else 0) < c2_trade[2] or (c2.resources[c1_trade[0]] if c1_trade[0] in c2.resources.keys() else 0) < \
                c1_trade[2]:
            return
        c1.resources[c1_trade[0]] = (c1.resources[c1_trade[0]] + c1_trade[2] if c1_trade[0] in c1.resources.keys() else (0 + 1) * c1_trade[2])
        c1.imports[c1_trade[0]] = (c1.imports[c1_trade[0]] + c1_trade[2] if c1_trade[0] in c1.imports.keys() else (0 + 1) * c1_trade[2])
        c2.resources[c1_trade[0]] = (c2.resources[c1_trade[0]] - c1_trade[2] if c1_trade[0] in c2.resources.keys() else (0 - 1) * c1_trade[2])
        c2.imports[c1_trade[0]] = (c2.imports[c1_trade[0]] - c1_trade[2] if c1_trade[0] in c2.imports.keys() else (0 - 1) * c1_trade[2])

        c2.resources[c2_trade[0]] = (c2.resources[c2_trade[0]] + c2_trade[2] if c2_trade[0] in c2.resources.keys() else (0 + 1) * c2_trade[2])
        c2.imports[c2_trade[0]] = (c2.imports[c2_trade[0]] + c2_trade[2] if c2_trade[0] in c2.imports.keys() else (0 + 1) * c2_trade[2])
        c1.resources[c2_trade[0]] = (c1.resources[c2_trade[0]] - c2_trade[2] if c2_trade[0] in c1.resources.keys() else (0 - 1) * c2_trade[2])
        c1.imports[c2_trade[0]] = (c1.imports[c2_trade[0]] - c2_trade[2] if c2_trade[0] in c1.imports.keys() else (0 - 1) * c2_trade[2])
...
def generate_magic():
    ...
    if Magic['Localization'] == "Localized":  # Get rid of all of the resource other than one random location
        select_regions = [r for r in RegionList if
                          hasattr(r, "resources") and (r.resources[Magic["Material"][0]] if Magic["Material"][0] in r.resources.keys() else 0) > 0]
        cl = CityList.copy()
        random.shuffle(cl)
        for c in CityList:
